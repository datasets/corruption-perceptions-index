...
tryCatch({
	...
	download.file(url2015, "raw_data/2015_CPI_DataMethodologyZIP.zip")
	unzip("raw_data/2015_CPI_DataMethodologyZIP.zip", exdir="raw_data")
	all_data<-read.xlsx("raw_data/Data & methodology/Data/CPI 2015_data.xlsx", 
	...
	write.csv(subset_data2015, "raw_data/CPI2015.csv", quote=F, row.names=F)
	message("cpi data 2015 succesfully downloaded and saved in raw_data/CPI2015.csv")
...
tryCatch({
	...
	download.file(url2014, "raw_data/CPI2014_DataBundle.zip")
	unzip("raw_data/CPI2014_DataBundle.zip", exdir="raw_data")
	all_data<-read.xlsx("raw_data/CPI2014_DataBundle/CPI 2014_Regional with data source scores_final.xlsx", 
	                    sheetIndex = 1, stringsAsFactors=F)
	subset_data2014<-all_data[-c(1,2), c(2,6)]
	subset_data2014[,1]<-sub(",", "", subset_data2014[,1])
	colnames(subset_data2014)<-c("Jurisdiction","CPI")
	write.csv(subset_data2014, "raw_data/CPI2014.csv", quote=F, row.names=F)
	message("cpi data 2014 succesfully downloaded and saved in raw_data/CPI2014.csv")
}, warning = function(cond) {
...
tryCatch({
	...
	download.file(url2013, "raw_data/CPI2013_DataBundle.zip")
	unzip("raw_data/CPI2013_DataBundle.zip", exdir="raw_data")
	all_data<-read.xlsx("raw_data/CPI2013_DataBundle/CPI2013_GLOBAL_WithDataSourceScores.xls", 
	                    sheetIndex = 1, stringsAsFactors=F)
	subset_data2013<-all_data[-1, c(2,7)]
	subset_data2013[,1]<-sub(",", "", subset_data2013[,1])
	colnames(subset_data2013)<-c("Jurisdiction", "CPI")
	write.csv(subset_data2013, "raw_data/CPI2013.csv", quote=F, row.names=F)
	message("cpi data 2013 succesfully downloaded and saved in raw_data/CPI2013.csv")
}, warning = function(cond) {
...
tryCatch({
	...
	download.file(url2012, "raw_data/2012_CPI_DataPackage.zip")
	unzip("raw_data/2012_CPI_DataPackage.zip", exdir="raw_data")
	all_data<-read.xlsx("raw_data/2012_CPI_DataPackage/CPI2012_Results.xls", 
	                    sheetIndex = 1, stringsAsFactors=F)
	subset_data2012<-all_data[-1, c(2,4)]
	subset_data2012[,1]<-sub(",", "", subset_data2012[,1])
	colnames(subset_data2012)<-c("Jurisdiction", "CPI")
	write.csv(subset_data2012, "raw_data/CPI2012.csv", quote=F, row.names=F)
	message("cpi data 2012 succesfully downloaded and saved in raw_data/CPI2012.csv")
}, warning = function(cond) {
...
tryCatch({
	...
	download.file(url2011, "raw_data/CPI2011_DataPackage.zip")
	unzip("raw_data/CPI2011_DataPackage.zip", exdir="raw_data")
	# below, if sheetIndex data is used, wrong cpi data are returned. 
	# use of sheetName works. not sure why.
	all_data<-read.xlsx2("raw_data/CPI2011_DataPackage/CPI2011_Results.xls", 
	...
	write.csv(subset_data2011, "raw_data/CPI2011.csv", quote=F, row.names=F)
	message("cpi data 2011 succesfully downloaded and saved in raw_data/CPI2011.csv")
}, warning = function(cond) {
...
tryCatch({
	...
	download.file(url2010, "raw_data/CPI2010.xls")
	all_data<-read.xlsx("raw_data/CPI2010.xls", sheetIndex = 1, stringsAsFactors=F)
	subset_data2010<-all_data[-c(1:3), c(2,3)]
	colnames(subset_data2010)<-c("Jurisdiction","CPI")
	subset_data2010[,1]<-sub(",", "", subset_data2010[,1])
	write.csv(subset_data2010,"raw_data/CPI2010.csv", quote=F, row.names=F)
	message("cpi data 2010 succesfully downloaded and saved in raw_data/CPI2010.csv")
}, warning = function(cond) {
...
for (year in 1998:2009) {
	...
	filename<-paste0("raw_data/CPI", year, ".csv")
...
for (year in 1998:2015) {
	data<-read.csv(paste0("raw_data/CPI", year, ".csv"), stringsAsFactor=F, header=T)
